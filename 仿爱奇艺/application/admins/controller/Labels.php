...
class Labels extends BaseAdmin{
    ...
    public  function channel() {
        $channel = $this->db->table('video_label')->field()->where(array('flag'=>channel))->lists();
        $this->assign('data',$channel);
        return view();
    }
    //资费
    public function charge() {
        $charge = $this->db->table('video_label')->field()->where(array('flag'=>charge))->lists();
        $this->assign('data',$charge);
        return view();
    }
    //地区
    public function area() {
        $area = $this->db->table('video_label')->field()->where(array('flag'=>area))->lists();
